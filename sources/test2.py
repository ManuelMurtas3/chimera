...
for id, fasta in fasta_transcripts.items():
	...
	for line in entries:
		...
		if entry.feature == "transcript":
			...
			if transcript_gene_id in test_transcipts:
				test_transcipts[transcript_gene_id] = test_transcipts[transcript_gene_id] + fasta
			else:
				test_transcipts[transcript_gene_id] = fasta

for transcript in transcripts:
	print("Transcript: " + transcript.transcript_id + " -> Gene: " + transcript.gene_id)
...
for transcript in transcripts:
	...
	transcript_dictionary[transcript.gene_id] = transcript.fasta_content
...
classification = open("../resources/gene-panel/classification.test.count", "w")
...
for read_id in read_transcripts.keys():
	...
	for i in range(0, read_length - kmer_size + 1):
		...
		gene_list.extend(dictionary_filter[kmer])
	classification.write(read_id + " classification: \n")
	for (id, fasta) in fasta_transcripts.items():
		if read_transcripts[read_id] in fasta:
			classification.write(id + " contains this read perfectly\n")
			position = fasta.index(read_transcripts[read_id])
			classification.write("\tposition in the transcript: " + str(position) + "\n")
	#gene_counter = Counter(gene_list)
	#for (gene_id, count) in gene_counter.most_common():
	#	classification.write("\t" + gene_id + ": " + str(count) + "\t")
	#	classification.write("is super string? ")
	#	if(read_transcripts[read_id] in test_transcipts[gene_id]):
	#		classification.write("yes\n")
	#	else:
	#		classification.write("no\n")
